...
public class Process_Function {
	public static ArrayList<String> Represent_LIST(String inputWord, String groupName, String docType, String dataPath, String jarPath) {
		final int TOPK=10;
		ArrayList<ArrayList<String>> lists = new ArrayList<ArrayList<String>>();
		ArrayList<HashMap<String,Double>> maps = new ArrayList<HashMap<String,Double>>();
	//	String path = "/home/hadoop/vectors/";
...
//			};
			...
			for(int i=0;i<3;i++){
				...
				HashMap<String, Double> newArrayMap = new HashMap<String, Double>();
				for(int k=0;k<TOPK;k++)
				{
					JSONObject jsonobj = (JSONObject)jsonArray.get(k);
					newArrayList.add(jsonobj.get("keyword").toString());
					newArrayMap.put(jsonobj.get("keyword").toString(), Double.parseDouble(  jsonobj.get("score").toString() ));
					//newArrayList.add(jsonobj.get("score").toString());
				}
				lists.add(newArrayList);
				maps.add(newArrayMap);
			...
			for(int i=0; i<3; i++) {
				...
				maps.get(i).keySet().removeAll(list_ThreeGroups);
				maps.get(i).keySet().removeAll(list_Two01);
				maps.get(i).keySet().removeAll(list_Two02);
				maps.get(i).keySet().removeAll(list_Two12);
				

			}
			

			ArrayList<String> Represent_list = new ArrayList<String>();

			Represent_list.addAll(list_ThreeGroups);
			Represent_list.addAll(list_Two01);
			Represent_list.addAll(list_Two02);
			Represent_list.addAll(list_Two12);
			
			HashMap<String,Double> temp = new HashMap<String,Double>();
			for(int i=0; i<3; i++) {
				temp.putAll(maps.get(i));
			}
			Iterator it = sortByValue(temp).iterator();
			while(Represent_list.size()<TOPK) {
				String key = (String) it.next();
				Represent_list.add(key);
			}
    ...
    public static void Result(ArrayList<String> PAPER, ArrayList<String> PATENT, ArrayList<String> REPORT) {
    	final int TOPK=10;
    	ArrayList<ArrayList<String>> lists = new ArrayList<ArrayList<String>>();
    	lists.add(PAPER);
    	lists.add(PATENT);
